...
namespace SilverCart.WebAPI.Hubs
{
    public class ChatHub : Hub
    {
        ...
        public ChatHub(IUnitOfWork unitOfWork, ICurrentTime currentTime, ILogger<ChatHub> logger, IClaimsService claimsService)
        {
        ...
        public async Task SendMessage(Guid conversationId, string content)
        {
            ...
            // Sử dụng transaction để tránh deadlock
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            _logger.LogInformation($"User Claims: {string.Join(", ", Context.User?.Claims?.Select(c => $"{c.Type}: {c.Value}") ?? Array.Empty<string>())}");
