...
{
    public static IQueryable<TEntity> CustomFilterV1<TEntity>(this IQueryable<TEntity> source, TEntity entity, string? keyword = null)
    {
        if (entity == null)
        {
        ...
        foreach (PropertyInfo property in entity.GetType().GetProperties())
        {
            ...
            if (property.PropertyType == typeof(string))
            {
                ...
                if (keyword != null)
                {
                    source = source.Where($"{propertyName}.ToLower().Contains(@0)", keyword);
                }
                else
                {
