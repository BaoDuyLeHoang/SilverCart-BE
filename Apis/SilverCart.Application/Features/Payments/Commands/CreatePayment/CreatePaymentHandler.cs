...
public class CreatePaymentHandler : IRequestHandler<CreatePaymentCommand, string>
{
    ...
    {
        // Validate wallet exists and get user info
        var wallet =
            await _unitOfWork.WalletRepository.GetByIdAsync(request.WalletId, w => w.User);
        if (wallet == null)
            throw new AppExceptions($"Ví của người dùng với ID {request.WalletId} không tồn tại", 404);

        // Validate user exists using CustomerUserRepository
        if (wallet.User != null)
        {
            var user = await _unitOfWork.CustomerUserRepository.GetByIdAsync(wallet.User.Id);
            if (user == null)
                throw new AppExceptions($"Người dùng với ID {wallet.User.Id} không tồn tại", 404);
        }
        ...
        string description =
            $"Nạp tiền vào ví {wallet.Id}|{paymentHistory.Id} - {request.Money} VND của user {wallet.User?.FullName} - {_currentTime.GetCurrentTime().ToString("dd/MM/yyyy")}";
