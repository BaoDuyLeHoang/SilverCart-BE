...
public class AddCartHandler : IRequestHandler<AddCartCommand, AddCartResponse>
{
    ...
    public async Task<AddCartResponse> Handle(AddCartCommand request, CancellationToken cancellationToken)
    {
        ...
        var user = await _unitOfWork.CustomerUserRepository.GetByIdAsync(request.CustomerUserId);
        if (user == null)
            throw new AppExceptions($"Không tìm thấy người dùng với ID {request.CustomerUserId}");

        // Create new Cart
        var cart = new Cart
        {
            UserId = request.CustomerUserId,
            ConsultantUserId = request.GuardianUserId,
            IsConsultantUserRecommend = request.ConsultantRecommendation,
            TotalPrice = 0
        };
